...
terminal = "urxvt -e zsh -c \"tmux -q has-session && exec tmux attach-session -d || exec tmux new-session -n$USER -s$USER@$HOSTNAME\""
...
-- {{{ Tags
tags = {
   names = { "main", "internet", "work", "media", "other"},
...
globalkeys = awful.util.table.join(
	...
	awful.key({ modkey,	"Control" }, "p", function() awful.util.spawn("scrot '%Y-%m-%d-%T-$wx$h.png' -e 'mv $f ~/Picture/screenshots/' -q 70") end)
)
